...
light = read_light(lightpath, max_frame)
more_list = []
            ...
            for fr in range(sl2):
                ...
                if real_frame in frame and id not in global_list:
                    ...
                    if order < NUsers:
                        ...
                        scene[fr: tot_len,order] = torch.stack([x[indices],y[indices],vx[indices],vy[indices],yaw_rad[indices],heading_rad[indices]],dim=1)
                        order +=1
                        break
                    else:
                        more += 1
